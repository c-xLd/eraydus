...
export function BlogIndex({ posts }: { posts: BlogPost[] }) {
  ...
  return <>
      ...
      <Link href={`/blog/${featured.slug}`} className="group grid overflow-hidden rounded-[2rem] bg-surface-dark text-white lg:grid-cols-[1.05fr_.95fr]">
        <div className="relative min-h-[360px] overflow-hidden lg:min-h-[520px]">
          {featured.featured_image ? (
            <img src={featured.featured_image} alt={featured.title} className="absolute inset-0 h-full w-full object-cover transition duration-700 group-hover:scale-105" />
          ) : (
    ...
    <section className="pb-32 md:pb-44"><div className="container mx-auto max-w-[1440px] px-6">
      ...
      {filteredPosts.length ? <div className="grid gap-x-8 gap-y-14 pt-10 md:grid-cols-2 lg:grid-cols-3">{filteredPosts.map((post) => <article key={post.id} className="group flex flex-col"><Link href={`/blog/${post.slug}`} className="block overflow-hidden rounded-2xl bg-surface"><div className="aspect-[4/3] overflow-hidden">{post.featured_image ? <img src={post.featured_image} alt={post.title} className="h-full w-full object-cover transition duration-700 group-hover:scale-105" /> : <div className="h-full w-full bg-surface flex items-center justify-center"><span className="text-xs text-muted-foreground uppercase tracking-widest opacity-50">Görsel Yok</span></div>}</div></Link><div className="pt-5"><div className="flex items-center gap-2 text-sm text-muted-foreground"><CalendarDays className="size-4" />{formatDate(post.published_at)}</div><h2 className="mt-4 text-2xl font-semibold tracking-tight transition-colors group-hover:text-champagne"><Link href={`/blog/${post.slug}`}>{post.title}</Link></h2><p className="mt-3 line-clamp-3 font-light leading-relaxed text-muted-foreground">{post.description}</p><Link href={`/blog/${post.slug}`} className="mt-6 inline-flex items-center gap-2 text-sm font-medium">Devamını oku <ArrowRight className="size-4 transition-transform group-hover:translate-x-1" /></Link></div></article>)}</div> : <div className="py-24 text-center"><p className="text-xl font-light">Aramanızla eşleşen bir yazı bulunamadı.</p><button onClick={() => { setActiveTag("Tümü"); setQuery("") }} className="mt-4 text-sm font-medium text-champagne">Filtreleri temizle</button></div>}
    </div></section>
  </>
}
