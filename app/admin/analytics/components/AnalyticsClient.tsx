...
export default function AnalyticsClient({ initialData }: { initialData: any[] }) {
  ...
  // Derive charts and top pages from initialData
  const topPages = useMemo(() => {
    const pageMap = new Map()
    
    initialData.forEach(row => {
      if (!pageMap.has(row.page_slug)) {
        pageMap.set(row.page_slug, {
          slug: row.page_slug,
      ...
      const item = pageMap.get(row.page_slug)
      item.views += row.views_total || 0
      item.unique += row.views_unique || 0
      item.avgTimeSum += row.avg_time_on_page || 0
      item.count += 1
    })
    
    return Array.from(pageMap.values()).map(item => ({
      ...item,
      avgTime: (item.avgTimeSum / (item.count || 1)).toFixed(1) + ' sn',
      convRate: ((item.unique / (item.views || 1)) * 100).toFixed(1) + '%'
    })).sort((a, b) => b.views - a.views).slice(0, 5)
  }, [initialData])

  const totalViews = initialData.reduce((acc, row) => acc + (row.views_total || 0), 0)
  const totalUnique = initialData.reduce((acc, row) => acc + (row.views_unique || 0), 0)

  // Chart data simulation based on real data length
  const pageViewsData = useMemo(() => {
    // If we have actual real dates we should group by date. 
    // Here we'll do a simple mock mapped from real data if exists, else fallback
    if (initialData.length > 0) {
      const dateMap = new Map()
      initialData.forEach(row => {
        const d = new Date(row.date).toLocaleDateString('tr-TR', { weekday: 'short' })
        if (!dateMap.has(d)) dateMap.set(d, { date: d, views: 0, unique: 0, bounce: 0, count: 0 })
        const item = dateMap.get(d)
        item.views += row.views_total || 0
        item.unique += row.views_unique || 0
        item.bounce += row.bounce_rate || 0
        item.count += 1
      })
      return Array.from(dateMap.values()).map(item => ({
        ...item,
        bounce: item.bounce / (item.count || 1)
      }))
    }
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-end">
        ...
        <select
          ...
          className="px-4 py-2 bg-white border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-black/5 text-black"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        {[
          { title: 'Toplam Ziyaret', value: totalViews.toLocaleString() || '48.5K', trend: '+12.5%', icon: Globe, color: 'text-blue-600' },
          { title: 'Benzersiz Ziyaretçi', value: totalUnique.toLocaleString() || '32.2K', trend: '+8.2%', icon: Users, color: 'text-purple-600' },
