...
const subjectOptions = [
  ...
  { value: 'proje', label: 'Proje & Özel Teklif' },
...
export function ContactClient() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setIsSubmitted(true)
      toast.success('Mesajınız başarıyla iletildi! Uzman ekibimiz en kısa sürede sizinle iletişime geçecektir.')
      setFormData({ name: '', email: '', phone: '', subject: '', message: '' })
    } catch (err: unknown) {
      console.error('Submit error:', err)
      const msg = err instanceof Error ? err.message : 'Mesajınız iletilirken bir sorun oluştu.'
      toast.error(msg)
    } finally {
    ...
    <div className="flex flex-col w-full">
      ...
      <section className="pt-40 pb-20 md:pt-48 md:pb-28 bg-background">
        <div className="container mx-auto px-6 max-w-[1440px]">
          ...
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-light tracking-tight max-w-4xl leading-[1.1]">
            Projenizi Birlikte <span className="font-semibold">Şekillendirelim</span>
          </h1>
          <p className="text-muted-foreground text-lg md:text-xl font-light mt-6 max-w-2xl">
            Hayalinizdeki banyo deneyimi için ilk adımı atın. Uzman ekibimiz size yardımcı olmaktan memnuniyet duyar.
          </p>
        </div>
      </section>
                  ...
                  <div key={item.label} className="group">
                    <div className="flex items-start gap-5">
                      <div className="w-12 h-12 rounded-full bg-surface flex items-center justify-center shrink-0 group-hover:bg-champagne/10 transition-colors duration-300">
                        <item.icon className="w-5 h-5 text-champagne" strokeWidth={1.5} />
                      </div>
                      <div>
        ...
        <div className="container mx-auto px-6 max-w-[1440px] py-20">
          <div>
            <div className="flex items-center gap-3 mb-8">
              <Building2 className="w-5 h-5 text-champagne" strokeWidth={1.5} />
