...
export function CollectionsClient({ products, categories = [], activeCategorySlug, title = 'Koleksiyonlar' }: CollectionsClientProps) {
  ...
  // Debounce search query changes to prevent layout thrashing and preserve input responsiveness (optimizing INP)
  useEffect(() => {
  ...
  // Optimization: Defined as a plain function rather than a React component and called via `{FilterContent()}`
  // to avoid unmounting and recreating DOM nodes on parent renders, which preserves input focus and enhances performance.
  const FilterContent = () => (
