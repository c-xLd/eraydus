...
export function CollectionsClient({ products, categories = [], activeCategorySlug, title = 'Koleksiyonlar' }: CollectionsClientProps) {
  ...
  // Decouple search input's text value from filtering state to optimize INP (prevent typing lag)
  const [localSearchQuery, setLocalSearchQuery] = useState('')
  const [selectedProfiles, setSelectedProfiles] = useState<string[]>([])
  const [priceRange, setPriceRange] = useState<number>(100000)

  // Debounce filter updates to searchQuery with a 200ms timeout hook to prevent layout thrashing
  useEffect(() => {
