...
import { ArrowRight, Search, Filter, SlidersHorizontal, Box, Check } from 'lucide-react'
...
export default function ShopClient() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return result
  }, [searchQuery, selectedCollections, selectedLayouts, priceRange, sortOrder])
    ...
    <div className="pt-32 pb-24 min-h-screen bg-background">
      <div className="container mx-auto px-6 max-w-[1440px]">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          <div>
            <h1 className="text-4xl md:text-5xl font-light tracking-tight mb-4">Mağaza</h1>
                ...
                <AnimatePresence mode="popLayout">
                  {filteredProducts.map((product) => (
                    <motion.div
                      ...
                      key={product.id}
                      className="group flex flex-col bg-surface rounded-3xl overflow-hidden border border-border hover:border-champagne/30 transition-colors shadow-sm hover:shadow-xl"
                    >
                      <div className="relative aspect-[4/3] overflow-hidden bg-muted">
                        ...
                        {product.isNew && (
                          <div className="absolute top-4 right-4">
                            <span className="bg-champagne text-black text-[10px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-full shadow-lg">
                              Yeni
                            </span>
                          </div>
                        )}
                      ...
                      <div className="p-6 flex flex-col flex-1">
                        <div className="flex justify-between items-start mb-2">
                          ...
                          <span className="text-lg font-medium text-champagne">
                        ...
                        <div className="mt-auto grid grid-cols-2 gap-3">
                          <Link 
                            ...
                            className="flex items-center justify-center py-2.5 rounded-lg border border-border text-sm font-medium hover:bg-muted transition-colors"
                          >
                            Detayları İncele
                          </Link>
                          <Link 
                            href={`/configurator?model=${product.id}`}
                            className="flex items-center justify-center py-2.5 rounded-lg bg-foreground text-background text-sm font-medium hover:bg-foreground/90 transition-colors group-hover:bg-champagne group-hover:text-black group-hover:shadow-[0_0_15px_rgba(201,168,106,0.3)]"
                          >
                            Konfigüre Et
                          </Link>
                        </div>
