...
cli.command("[root]", "start dev server").alias("serve").alias("dev").option("--host [host]", `[string] specify hostname`, { type: [convertHost] }).option("--port <port>", `[number] specify port`).option("--open [path]", `[boolean | string] open browser on startup`).option("--cors", `[boolean] enable CORS`).option("--strictPort", `[boolean] exit if specified port is already in use`).option(
      ...
      plugins: [{
          ...
          async transform(code, id) {
            ...
            try {
              ...
              const injectionUrl = await import.meta.resolve("file:///c:/Users/ahmet/.antigravity-ide/extensions/jonpena.console-warrior-1.3.31-universal/dist/injectionCode.js");
  ...
  const green = (txt) => `[32m${txt}[0m`;
	const magentaBold = (txt) => `[35;1m${txt}[0m`;
  console.log(`${green("Console Warrior ⚔️  connected to ")}${magentaBold("VITE")}`);
	console.log((`${green("------------------------------------")}`));
  /* console-warrior-websocket end */    const info = server.config.logger.info;
